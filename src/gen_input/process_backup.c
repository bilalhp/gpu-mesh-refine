...
int read_tri_points(FILE *infile)
{
	...
	for (int i = 0; i < count; i++) {
		...
		for (int j = 0; j < 3; j++) {
			lineptr = findfield(lineptr);
			tp.point[j] = strtod(lineptr, &lineptr);
		}
...
int write_triangles(FILE *outfile)
{
	...
	r = get_rand_int(0, tri_count/4);

	for (int i = 0; i < tri_count; i++) {
		fprintf(outfile, "%d %d %d %d\n", T(i).edge_ix[0], T(i).edge_ix[1], T(i).edge_ix[2], 1);
	}
...
int edge_equals(edge_t *e1, edge_t *e2)
{
...
int main(int argc, char *argv[])
{
	...
	read_nodes(nodefile);
	read_ele(elefile);
	create_edges_and_triangles();
	write_points(outfile);
	write_edges(outfile);
	write_triangles(outfile);
